...
@pytest.mark.asyncio
async def test_bolus_calc_round_step_zero_from_db(monkeypatch, tmp_path):
    called = {}
    now = datetime.now(timezone.utc)
    iob_info = IOBInfo(
        iob_u=0.0,
    ...
    async def fake_get_user_settings_service(username, session):
        settings = UserSettings.default().model_dump()
        settings["round_step_u"] = 0.0
        return {"settings": settings}

    def fake_load_settings(self, username: str = "admin"):
        raise AssertionError("store.load_settings should not be called")

    def fake_calculate_bolus_v2(*, request, settings, iob_u, glucose_info, autosens_ratio, autosens_reason):
        called["round_step_u"] = settings.round_step_u
        return BolusResponseV2(
            meal_bolus_u=0.0,
            correction_u=0.0,
            iob_u=0.0,
    ...
    monkeypatch.setattr(
        "app.services.settings_service.get_user_settings_service",
        fake_get_user_settings_service,
    )
    monkeypatch.setattr(
        "app.services.bolus_calc_service.compute_iob_from_sources",
        fake_compute_iob_from_sources,
    ...
    monkeypatch.setattr("app.services.bolus_calc_service.calculate_bolus_v2", fake_calculate_bolus_v2)
    monkeypatch.setattr(DataStore, "load_settings", fake_load_settings)
