...
async def test_status_success():
    route = respx.get("https://example.com/api/v1/status.json").mock(
...
async def test_latest_sgv_parses():
    respx.get("https://example.com/api/v1/entries/sgv.json").mock(
        return_value=httpx.Response(
            200,
            json=[{"sgv": 123, "direction": "Flat", "date": 1690000000000, "delta": 1.1}],
        )
    )
    client = NightscoutClient(settings=None, client=httpx.AsyncClient(base_url="https://example.com"))
...
async def test_recent_treatments_query():
    ...
    route = respx.get("https://example.com/api/v1/treatments.json").mock(
        return_value=httpx.Response(
            200,
            json=[
                {
...
async def test_status_failure_raises():
    respx.get("https://example.com/api/v1/status.json").mock(return_value=httpx.Response(500, json={}))
    client = NightscoutClient(settings=None, client=httpx.AsyncClient(base_url="https://example.com"))
