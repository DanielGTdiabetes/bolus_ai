...
class BolusParams(BaseModel):
    """Legacy client-side dosing snapshot.

    Kept only for backwards compatibility with older clients. The authoritative
    recalc service no longer uses these values to determine a dose; current
    user settings are resolved by the backend central bolus service.
    """
    ...
    insulin_curve: Literal["walsh", "bilinear", "fiasp", "novorapid", "linear"] = "walsh"
    peak_minutes: int = 75


class NightscoutConn(BaseModel):
    """Legacy request-scoped Nightscout connection.

    Retained so old clients continue to validate. The recalc service resolves
    the authenticated user's configured glucose/treatment sources centrally.
    """
...
class RecalcSecondRequest(BaseModel):
    ...
    # Backward-compatible inputs. They are accepted but not authoritative.
    params: Optional[BolusParams] = None
    nightscout: Optional[NightscoutConn] = None
