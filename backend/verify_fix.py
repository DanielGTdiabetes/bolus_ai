...
async def verify_nightscout_client():
    ...
    try:
        # Check internal state of client (white-box test)
        print(f"Client Headers (Auth should be empty): {client.client.headers.get('Authorization')}")
        print(f"Client Headers (API-SECRET should be empty): {client.client.headers.get('API-SECRET')}")
        print(f"Client Params (Should have token): {client.client.params}")
