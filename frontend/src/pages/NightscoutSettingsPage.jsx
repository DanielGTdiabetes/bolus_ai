...
export default function NightscoutSettingsPage() {
    ...
    const handleDelete = async () => {
        try {
            ...
            setMessage('Error al eliminar credenciales');
        }
    };

    return (
        <div className="nightscout-settings container mx-auto p-4">
            <h1 className="text-2xl font-bold mb-4">Configuración Nightscout</h1>
            {message && <p className="mb-2 text-green-600">{message}</p>}
            <form onSubmit={handleSave} className="space-y-4">
                <div>
                    <label className="block font-medium">URL de Nightscout</label>
                    <input
                        type="text"
                        value={url}
                        onChange={(e) => setUrl(e.target.value)}
                        className="w-full border rounded p-2"
                        placeholder="https://mi-nightscout.example.com"
                        required
                    />
                </div>
                <div>
                    <label className="block font-medium">Clave API (API Secret)</label>
                    <input
                        type="password"
                        value={token}
                        onChange={(e) => setToken(e.target.value)}
                        className="w-full border rounded p-2"
                        placeholder="Tu token o secreto de Nightscout"
                        required
                    />
                </div>
                <div className="flex items-center">
                    <input
                        ...
                        className="mr-2"
                    />
                    <label htmlFor="enabled" className="font-medium">Habilitado</label>
                </div>
                <button
                    type="submit"
